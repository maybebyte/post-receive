...
subtest 'website_md push fails fast with a clear error when ssg6 is missing' =>
	sub {
		...
		my $preserved_marker =
		seed_preserved_webroot_marker( $harness, 'preserved-before-run.txt', );
		...
		like( $result->{stderr}, qr/\Q$expected_ssg_path\E/,
...
subtest 'website_md push fails fast with a clear error when rssg is missing' =>
	sub {
		my $harness = PostReceive::TestHarness->new;
		$harness->install_fake_stagit;
		...
		like( $result->{stderr}, qr/\Q$expected_rssg_path\E/,

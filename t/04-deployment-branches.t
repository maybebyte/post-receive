...
subtest 'append_to_work_clone rejects invalid file lists before running git' =>
	sub {
		...
		like(
			$@,
			qr/append_to_work_clone requires a non-empty files array reference/,
		...
		like(
			$@,
			qr/append_to_work_clone file_rel must stay within the work clone/,
			'append validation names the escaping file_rel'
		) or diag( $harness->workspace_diag );
		is(
			scalar @{ $harness->command_results },
			$initial_command_count,
...
subtest
	...
	SKIP: {
			skip 'deployed sysadm fixture files missing', 2
			unless -f $readme_path && -f $nested_file_path;
			like_with_diag(
	...
	SKIP: {
			skip 'log.html or index.html missing', 1
...
subtest
	...
	=> sub {
		...
		my $run_diag = join( "\n",
			...
			'ssg6_mode_after_run: '
			. ( $harness->file_mode_octal($ssg6_target_path) // '(missing)' ),
			'rssg_mode_after_run: '
			. ( $harness->file_mode_octal($rssg_target_path) // '(missing)' ),
		);
		...
		like_with_diag(
			...
			'shared publishing output stays under the temp webroot', $run_diag,
		);
	...
	SKIP: {
			skip 'isolated dotfiles helper replacements missing', 4
			unless -f $ssg6_target_path && -f $rssg_target_path;
			is_with_diag(

...
my $assets = setup_or_stop( 'seed shared stagit assets',
	sub { $harness->seed_stagit_assets }, $harness, );
setup_or_stop( 'install fake stagit',
...
subtest
	'public helper surface exposes workspace, file, executable, trace, and mode mechanics'
	=> sub {
		my $workspace_diag = $harness->describe_workspace;

		is_with_diag(
			$harness->workspace_diag, $workspace_diag,
			'workspace_diag aliases describe_workspace', $workspace_diag,
		);
		...
		{
			like_with_diag( $workspace_diag, qr/^\Q$diag_line\E$/m,
				"describe_workspace includes $diag_line",
				$workspace_diag, );
		}

		my $ensured_dir =
		$harness->ensure_dir( catdir(qw(helper-check nested parent)) );
		ok_with_diag(
			-d $ensured_dir,
			'ensure_dir creates a nested relative directory',
			$workspace_diag
		);
		like_with_diag(
			$ensured_dir,
		...
		ok_with_diag(
			-f $text_path,
			'write_file creates missing parent directories for relative paths',
			$workspace_diag
		);
		is_with_diag(
		...
		is_with_diag(
			$harness->read_file($binary_path), $binary_payload,
			'write_file preserves binary content', $workspace_diag,
		);
		...
		is_with_diag(
			$harness->executable_on_path(
				'helper-tool', path => catdir(qw(helper-check bin))
			),
			$helper_tool_path,
		...
		is_with_diag(
			$harness->executable_in_dir(
				catdir(qw(helper-check bin)),
				'helper-tool'
			),
			$helper_tool_path,
		...
		ok_with_diag(
			!defined $harness->executable_in_dir(
				catdir(qw(helper-check bin)),
				'missing-helper'
			),
		...
		is_with_diag(
			$harness->file_mode_octal(
				catfile(qw(helper-check bin helper-tool))
			),
			'0700',
		...
		ok_with_diag(
			!defined $harness->file_mode_octal(
				catfile(qw(helper-check missing mode.txt))
			),
...
is_with_diag( $result->{command}->[0],
	$harness->hook_path, 'hook ran via executable child path', $run_diag );
...
SKIP: {
	...
	like_with_diag( $owner, qr/Ashlen/, 'owner metadata names Ashlen',
		$run_diag );
	like_with_diag(
		$description,
		qr/Learning Perl exercises/,
		'description metadata names Learning Perl exercises', $run_diag,
	);
	like_with_diag( $url, qr/\Q$expected_url\E/,
		'url metadata uses the public clone URL', $run_diag );
}
